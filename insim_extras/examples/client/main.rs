...
use insim_extras::client::handle::Handle;
...
pub async fn main() -> Result<()> {
    ...
    let (h, j) = Handle::new(client);

    let res = h
        .send_and_await_with_request_identifier(Tiny {
            subt: TinyType::Ping,
            ..Default::default()
        })
        .await;

    tracing::info!("with reqi = {:?}", res);
    ...
    let mut firehose = h.stream().await;

    let mut i = 0;

    while let Ok(packet) = firehose.recv().await {
        tracing::info!("{:?}", packet);
