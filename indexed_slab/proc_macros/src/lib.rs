...
pub fn indexed_slab(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let fields = match input.data {
        ...
        _ => abort_call_site!("IndexedSlab only supports structs as elements"),
    };
    ...
    // Filter out all the fields that do not have a indexed_slab attribute, so we can ignore the non-indexed fields.
    let fields_to_index = || {
        named_fields.named.iter().filter(|f| {
            f.attrs.first().is_some() && f.attrs.first().unwrap().path.is_ident("indexed_slab")
        })
    };
    ...
    let lookup_table_fields = fields_to_index().map(|f| {
        ...
        let (ordering, uniqueness, _optional) = get_index_kind(f).unwrap_or_else(|| {
            abort_call_site!("Attributes must be in the style #[indexed_slab(hashed_unique)]")
        });

        match uniqueness {
            Uniqueness::Unique => match ordering {
                Ordering::Hashed => quote! {
                    #index_name: indexed_slab::rustc_hash::FxHashMap<#ty, usize>,
                },
            ...
            Uniqueness::NonUnique => match ordering {
                Ordering::Hashed => quote! {
                    #index_name: indexed_slab::rustc_hash::FxHashMap<#ty, Vec<usize>>,
                },
                Ordering::Ordered => quote! {
    ...
    let inserts: Vec<proc_macro2::TokenStream> = fields_to_index()
        .map(|f| {
            ...
            let (_ordering, uniqueness, optional) = get_index_kind(f).unwrap_or_else(|| {
                abort_call_site!("Attributes must be in the style #[indexed_slab(hashed_unique)]")
            });

            match (uniqueness, optional) {
                (Uniqueness::Unique, false) => quote! {
                ...
                (Uniqueness::Unique, true) => quote! {
                    if elem.#field_name.is_some() {
                        let orig_elem_idx = self.#index_name.insert(elem.#field_name.clone(), idx);
                        panic!("Unable to insert element, uniqueness constraint violated on field '{}'", #field_name_string);
                    }
                },
                (Uniqueness::NonUnique, _) => quote! {
    ...
    let map_name = format_ident!("IndexedSlab{}", element_name);
    ...
    let accessors = fields_to_index().map(|f| {
        ...
        let getter = match (&uniqueness, optional) {
            (Uniqueness::Unique, false) => quote! {
...
fn get_index_kind(f: &syn::Field) -> Option<(Ordering, Uniqueness, bool)> {
    // eeew?
    let optional = f.to_token_stream().to_string().contains("Option < ");
    ...
    // FIXME change this to allow for the form #[indexed_slab(hashed,unique)]
    // assume absence of unique means non-unique
    // add indicator of how to manage/deal with None

    if nested_path.is_ident("hashed_unique") {
        Some((Ordering::Hashed, Uniqueness::Unique, optional))
    } else if nested_path.is_ident("ordered_unique") {
        Some((Ordering::Ordered, Uniqueness::Unique, optional))
    } else if nested_path.is_ident("hashed_non_unique") {
        Some((Ordering::Hashed, Uniqueness::NonUnique, optional))
    } else if nested_path.is_ident("ordered_non_unique") {
        Some((Ordering::Ordered, Uniqueness::NonUnique, optional))
    } else {
        None
    }
}
