...
use crate::{
    App, AppError, ChatParser, Connected, Dispatch, Event, Packet, Presence, Sender,
...
/// Pull an app apart and drive one dispatch directly.
async fn drive(app: App<TestState>, state: &TestState, d: Dispatch) {
    ...
    dispatch_cycle(d, state, &sender, &chain, &handlers, &extensions).await;
}
...
#[tokio::test]
async fn sender_emit_pushes_into_back_channel() {
    // Confirms that Sender::send/emit reach the dispatcher's back-channel — the
    // serve loop drains this channel on its own; here we just verify the
    // command shows up.
    let (cmd_tx, mut cmd_rx) = mpsc::channel::<Command>(8);
    let sender = Sender::new(cmd_tx);

    sender.emit("hello".to_string()).await.expect("emit ok");
