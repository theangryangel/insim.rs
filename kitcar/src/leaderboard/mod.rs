...
impl Leaderboard {
    ...
    pub fn add_player_score(&mut self, plid: &PlayerId, points: i32) -> i32 { 
        self.scores.entry(*plid).and_modify(|p| *p = p.saturating_add(points)).or_insert(points).clone()
    }
    ...
    pub fn ranking(&self, truncate_to: Option<usize>) -> Vec<(PlayerId, i32)> {
        let mut ordered = self.scores.iter().map(|(k, v)| {
            (k.clone(), v.clone())
...
impl LeaderboardHandle {
    ...
    pub async fn add_player_score(&self, plid: &PlayerId, score: i32) -> i32 {
        ...
        let _ = self.query_tx.send(LeaderboardQuery::AddPlayerScore { plid: *plid, score, response_tx: tx }).await;
    ...
    pub async fn player_score(&self, plid: &PlayerId) -> Option<i32> {
        let (tx, rx) = oneshot::channel();
        let _ = self.query_tx.send(LeaderboardQuery::GetPlayerScore { plid: *plid, response_tx: tx }).await;
        rx.await.unwrap_or_default()
    }
    ...
    pub async fn ranking(&self, truncate_to: Option<usize>) -> Vec<(PlayerId, i32)> {
        let (tx, rx) = oneshot::channel();
        let _ = self.query_tx.send(LeaderboardQuery::GetRanking { truncate_to, response_tx: tx }).await;
        rx.await.unwrap_or_default()
    }
