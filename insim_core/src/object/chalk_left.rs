...
impl ObjectVariant for ChalkLeft {
...
mod tests {
    ...
    #[test]
    fn test_chalk_left_round_trip() {
        let original = ChalkLeft::default();
        let wire = original.to_wire().expect("to_wire failed");
        let decoded = ChalkLeft::from_wire(wire).expect("from_wire failed");
