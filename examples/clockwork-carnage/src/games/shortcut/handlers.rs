use insim::{
    ...
    insim::{PlayerType, Uco},
};
use kitcar::{
    AppError, Connected, Disconnected, Event, Packet, PlayerLeft, PlayerTeleportedToPits,
    RoundEndReason, RoundEnded, RoundManager, RoundStarted, Sender, State, TakingOver, World, mtc,
};
...
/// still connected.
async fn abandon_run(state: &State<Shortcut>, ui: &ShortcutUi, plid: PlayerId) {
    let run = state.write().active_runs.remove(&plid);
    if let Some((ucid, uname, _start)) = run {
        let best_time_ms = state
            .read()
            .leaderboard
            .iter()
            .find(|e| e.0 == uname)
