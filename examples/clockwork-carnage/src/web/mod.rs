...
pub async fn serve(listen: SocketAddr, pool: db::Pool, cfg: WebConfig) -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/sessions/{id}/standings/climb/best", get(session_climb_best))
        .route("/sessions/{id}/standings/climb/all", get(session_climb_all))
        .route("/sessions/{id}/start", post(session_start))
        .route("/sessions/{id}/complete", post(session_complete))
