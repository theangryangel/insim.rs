...
impl Encode for ConInfo {
    fn encode(&self, buf: &mut bytes::BytesMut) -> Result<(), insim_core::EncodeError> {
        ...
        if self.thr > 15 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: 15, found: self.thr as usize }.context("Thr out of range"));
        }

        if self.brk > 15 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: 15, found: self.brk as usize }.context("Brk out of range"));
        }
        ...
        if self.clu > 15 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: 15, found: self.clu as usize }.context("Clu out of range"));
        }

        if self.han > 15 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: 15, found: self.han as usize }.context("Han out of range"));
        }
        ...
        if self.gearsp > 15 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: 15, found: self.gearsp as usize }.context("Gearsp out of range"));
        }
...
impl Encode for Con {
    fn encode(&self, buf: &mut bytes::BytesMut) -> Result<(), insim_core::EncodeError> {
        ...
        match TryInto::<u32>::try_into(self.time.as_millis()) {
            ...
            Err(_) => return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: u32::MAX as usize, found: self.time.as_millis() as usize }.context("Time out of range")),
        }
        self.a.encode(buf)?;
        self.b.encode(buf)?;
        Ok(())
