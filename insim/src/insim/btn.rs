    ...
    fn decode(buf: &mut bytes::Bytes) -> Result<Self, insim_core::DecodeError> {
        ...
        let (caption, text) = if let Some(&0_u8) = buf.first() {
            ...
            let split = if let Some(split) = buf.iter().position(|c| c == &0_u8) {
                ...
                return Err(DecodeErrorKind::ExpectedNull.context("Expected caption in BTN text field, but found no \0"));
            };
...
impl Encode for Btn {
    fn encode(&self, buf: &mut bytes::BytesMut) -> Result<(), insim_core::EncodeError> {
        if self.l > 200 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 1, max: 200, found: self.l as usize }.context("Length out of range on BTN"));
        }

        if self.t > 200 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: 200, found: self.t as usize }.context("Top out of range"));
        }

        if self.w > 200 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 1, max: 200, found: self.w as usize }.context("Width out of range"));
        }

        if self.h > 200 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: 200, found: self.t as usize }.context("Height out of range"));
        }
        ...
        if let Some(caption) = &self.caption {
            ...
            if (caption.len() + text.len()) > (BTN_TEXT_MAX_LEN - 2) {
                return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: BTN_TEXT_MAX_LEN - 2, found: caption.len() + text.len() }.context("Caption + text too large"));
            }
...
impl Decode for Btt {
    fn decode(buf: &mut bytes::Bytes) -> Result<Self, insim_core::DecodeError> {
        ...
        let typein = u8::decode(buf)?;
        if typein > 96 {
            return Err(insim_core::DecodeErrorKind::OutOfRange { min: 0, max: 96, found: typein as usize }.context("Typein too large"));
        }
        let typein = if typein > 0 { Some(typein) } else { None };
        buf.advance(1);
...
impl Encode for Btt {
    fn encode(&self, buf: &mut bytes::BytesMut) -> Result<(), insim_core::EncodeError> {
        ...
        if typein > 96 {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: 96, found: typein as usize }.context("Typein too large"));
        }
        typein.encode(buf)?;
        buf.put_bytes(0, 1);
