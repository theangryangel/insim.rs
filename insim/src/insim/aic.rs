    ...
    fn encode(&self, buf: &mut bytes::BytesMut) -> Result<(), insim_core::EncodeError> {
        ...
        if let Some(time) = self.time {
            match u8::try_from(time.as_millis() / 10) {
                ...
                Err(_) => return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: u8::MAX as usize, found: (time.as_millis() / 10) as usize }.context("Time too large on AIC")),
            }
        } else {
            0_u8.encode(buf)?;
        }
...
impl Encode for Aic {
    fn encode(&self, buf: &mut bytes::BytesMut) -> Result<(), insim_core::EncodeError> {
        ...
        if self.inputs.len() > AIC_MAX_INPUTS {
            return Err(insim_core::EncodeErrorKind::OutOfRange { min: 0, max: AIC_MAX_INPUTS, found: self.inputs.len() }.context("Too many inputs on AIC"));
        }
