"""
TestClient — a drop-in replacement for ``InsimClient`` that lets you inject
...
class TestClient:
    """Minimal test double for ``InsimClient``.
    ...
    No network connection, no Rust runtime — everything is synchronous-
    compatible and driven by ``inject()``.
    """
    ...
    def include_handler(self, handler: Handler) -> None:
        """Register a handler, mirroring ``InsimClient.include_handler``."""
        self._handlers.append(handler)

    def middleware(self, fn: MiddlewareFn) -> MiddlewareFn:
        """Register middleware, mirroring ``InsimClient.middleware``."""
    ...
    async def inject(self, raw_json: str) -> None:
        ...
        This is the primary test entry-point.  Pass any JSON string that the
        Rust FFI would produce (including the ``"type"`` discriminator).
        """
